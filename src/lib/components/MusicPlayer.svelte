<script lang="ts">
    ...
    const STYLES: StyleGroup[] = [
        {
            ...
            tracks: [
                ...
                { title: "It's Like That", path: "/audio/music/90s/It's Like That - Run–D.M.C., Jason Nevins.m4a" },
                { title: "It's Tricky", path: "/audio/music/90s/It's Tricky - Run–D.M.C..m4a" },
                { title: "Lovefool", path: "/audio/music/90s/Lovefool - The Cardigans.m4a" },
                { title: "No Diggity", path: "/audio/music/90s/No Diggity - Blackstreet, Dr. Dre, Queen Pen.m4a" },
                { title: "Pump Up The Jam", path: "/audio/music/90s/Pump Up The Jam - Technotronic.m4a" },
                { title: "Rhythm Is a Dancer", path: "/audio/music/90s/Rhythm Is a Dancer - SNAP!.m4a" },
                { title: "Spice Up Your Life", path: "/audio/music/90s/Spice Up Your Life - Spice Girls.m4a" },
                { title: "The Rhythm Of The Night", path: "/audio/music/90s/The Rhythm Of The Night - Corona.m4a" },
                { title: "What Is Love", path: "/audio/music/90s/What Is Love - 7 Mix - Haddaway.m4a" },
            ],
        },
        {
            label: "METAL",
            ...
            tracks: [
                ...
                { title: "No", path: "/audio/music/metal/No - Fleshgod Apocalypse.m4a" },
                { title: "Roundabout", path: "/audio/music/metal/Roundabout - Allegaeon.m4a" },
                { title: "The Dopamine Void Pt. II", path: "/audio/music/metal/The Dopamine Void, Pt. II - Allegaeon.m4a" },
                { title: "The Fool", path: "/audio/music/metal/The Fool - Fleshgod Apocalypse.m4a" },
                { title: "War is the Answer", path: "/audio/music/metal/War is the Answer - Five Finger Death Punch.m4a" },
            ],
        },
        {
            label: "WEEB",
            ...
            tracks: [
                ...
                { title: "Mirage", path: "/audio/music/weeb/Mirage - Creepy Nuts.m4a" },
                { title: "chase", path: "/audio/music/weeb/chase - batta.m4a" },
                { title: "mnemonic", path: "/audio/music/weeb/mnemonic - Intervals.m4a" },
                { title: "Yofukashino Uta", path: "/audio/music/weeb/よふかしのうた - Yofukashino Uta - Creepy Nuts.m4a" },
    ...
    function buildShuffledQueue(styleKey: string, excludeIndex: number = -1): number[] {
        ...
        if (excludeIndex !== -1 && indices[0] === excludeIndex && indices.length > 1) {
    ...
    function playNext() {
        ...
        if (queuePos >= shuffledQueue.length) {
            shuffledQueue = buildShuffledQueue(currentStyleKey, shuffledQueue[shuffledQueue.length - 1]);
            queuePos = 0;
        }
...
<div class="music-bar" class:expanded={isExpanded}>
    ...
    <div class="bar-header" role="button" tabindex="0" on:click={toggleExpand} on:keydown={(e) => e.key === 'Enter' && toggleExpand()}>
        ...
        <div class="bar-controls-quick" role="none" on:click|stopPropagation on:keydown|stopPropagation>
            <button class="ctrl-btn" on:click={playPrev} title="Previous" aria-label="Previous track">
                <svg viewBox="0 0 24 24" fill="currentColor" width="14" height="14"><path d="M6 6h2v12H6zm3.5 6 8.5 6V6z"/></svg>
            </button>
            <button class="ctrl-btn ctrl-play" on:click={togglePlay} title={isPlaying ? 'Pause' : 'Play'} aria-label={isPlaying ? 'Pause' : 'Play'}>
                {#if isPlaying}
                    <svg viewBox="0 0 24 24" fill="currentColor" width="16" height="16"><path d="M6 19h4V5H6zm8-14v14h4V5z"/></svg>
                {:else}
                    <svg viewBox="0 0 24 24" fill="currentColor" width="16" height="16"><path d="M8 5v14l11-7z"/></svg>
                {/if}
            </button>
            <button class="ctrl-btn" on:click={playNext} title="Next" aria-label="Next track">
                <svg viewBox="0 0 24 24" fill="currentColor" width="14" height="14"><path d="M6 18l8.5-6L6 6zm8.5-6 2.5 1.5V8.5L14.5 12z"/><path d="M16 6h2v12h-2z"/></svg>
            </button>
            <button class="ctrl-btn" on:click={toggleMute} title={isMuted ? 'Unmute' : 'Mute'} aria-label={isMuted ? 'Unmute' : 'Mute'}>
                {#if isMuted || volume === 0}
                ...
                {:else if volume < 0.5}
                    <svg viewBox="0 0 24 24" fill="currentColor" width="14" height="14"><path d="M18.5 12A4.5 4.5 0 0 0 16 7.97v8.05c1.48-.73 2.5-2.25 2.5-4.02zM5 9v6h4l5 5V4L9 9H5z"/></svg>
                {:else}
                    ...
                    <button
                        ...
                        on:click={() => {
                            ...
                            if (idx !== -1) {
                                ...
                                shuffledQueue = [i, ...shuffledQueue.slice(queuePos + 1)];
                                queuePos = 0;
                            }
